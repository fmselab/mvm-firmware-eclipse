...
uint16_t ADC_ADS1115::readADC_SingleEnded(uint8_t channel) {
  ...
  dbg->DbgPrint(DBG_KERNEL, DBG_VALUE,
                "CHANNEL:  " + String(channel) + "   VALUE:  " + String(value));
...
bool ADC_ADS1115::asyncMeasure(uint8_t channel) {
  ...
  if (__pending_meas) return false;
...
bool ADC_ADS1115::asyncGetResult(uint32_t* value) {
  ...
  if (hwi->Get_dT_millis(__last_millis) < m_conversionDelay) return false;
